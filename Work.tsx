  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className={`w-full h-full pointer-events-none transition-transform duration-700 ease-in-out flex items-center justify-center ${isHovered ? 'scale-100' : 'scale-110 grayscale opacity-80'}`}>
            <Player 
              url={project.videoUrl} 
              playing={isHovered}
              muted 
...
export default function Work() {
  ...
  // Suppress "The play() request was interrupted by a call to pause()" errors
  useEffect(() => {
    const handler = (e: PromiseRejectionEvent) => {
      const msg = e.reason?.message || '';
      if (msg.includes('The play() request was interrupted')) {
        e.preventDefault(); // hide DOMException from console
      }
    };
    window.addEventListener('unhandledrejection', handler);

    // Monkey-patch console.error to also hide it if ReactPlayer logs it explicitly
